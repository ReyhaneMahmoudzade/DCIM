...
export default function Rack() {
  ...
  const columns = [
    ...
    { field: 'parent', headerName: 'Parent Datacenter', width: 150, editable: true },
  ...
  useEffect(() => {
    const fetchData = async () => {
      const response = await fetch('https://dcim-bb92a-default-rtdb.firebaseio.com/rack.json');
  ...
  const handleDelete = async (id) => {
    ...
    await fetch(`https://dcim-bb92a-default-rtdb.firebaseio.com/rack/${key}.json`, {
      method: 'DELETE',
    });

    setRack((prev) => prev.filter((row) => row.id !== id));
  };
